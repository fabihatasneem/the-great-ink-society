...
@RestController
public class GetMostRecentPodcasts {

    @Autowired
    private PodcastSeriesRepository podcastSeriesRepository;

    @RequestMapping(value = "/getMostRecentPodcasts", method = RequestMethod.POST)
    public List<PodcastSeries> getMostRecentPodcasts(){
        Pageable limitedData = PageRequest.of(0, 3, Sort.by("creationDate").descending());
        int publishStatus = 1;

        return podcastSeriesRepository.findByPublishStatusOrderByCreationDateDesc(limitedData, publishStatus);
    }
